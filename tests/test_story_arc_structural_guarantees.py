...
class StoryArcStructuralGuaranteeTests(unittest.TestCase):
    ...
    def test_majority_story_gets_exact_sequence_budget(self):
        ...
        self.assertIn("Create as many distinct ordinary instances", repair_prompt)
        self.assertIn("do not assume the few instances already named", repair_prompt)
        self.assertIn("do not split or kill the same instance twice", repair_prompt)
        self.assertIn("START of the first counted process beat", repair_prompt)
