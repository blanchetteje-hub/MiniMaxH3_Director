"""GPT-OSS formatting and validation for MiniMax H3 prompt fields.
...
def _strip_markdown(value: str) -> str:
    ...
    # MiniMax consumes prose rather than Markdown.  GPT-OSS may wraps a
...
def _parse_labeled_text(text: str) -> dict[str, Any]:
    ...
    if not matches:
        raise ValueError("GPT-OSS response is neither JSON nor labeled H3 fields.")
    parsed: dict[str, Any] = {}
    ...
    if isinstance(llm_result, Mapping):
        ...
        try:
            ...
            if not isinstance(decoded, dict):
                raise ValueError("GPT-OSS JSON response must be an object.")
            result = decoded
    else:
        raise TypeError("GPT-OSS response must be a mapping or text.")
...
def format_gpt_prompt(llm_result: Any, context: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    because semantic problems remain; call :func:`validate_gpt_prompt`
...
def validate_gpt_prompt(
    ...
    safe_context: Mapping[str, Any] = context or {}
    if not isinstance(result, Mapping):
        return ["Formatted GPT-OSS result must be an object."]
    issues: list[str] = []
    for rule in RULE_REGISTRY:
...
class GPTFormatter(BaseFormatter):
    ...
    def format_prompt(
        ...
        return format_gpt_prompt(llm_result, context)
    ...
    def validate_prompt(
        ...
        return validate_gpt_prompt(result, context)
