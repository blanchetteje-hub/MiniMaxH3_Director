...
# Semantic issue types are never sufficient on their own to bypass verification.
# Deterministic Python proofs are supplied explicitly to issue_can_bypass_verification.
DETERMINISTIC_VERIFIER_BYPASS_PROOFS = frozenset({
    "invalid_beat_range",
    "nonexistent_phase",
    "duplicate_beat_id",
...
_THREAT_ID_PATTERN = re.compile(r"^threat_(?P<number>\d+)$", re.IGNORECASE)
_RETRIEVAL_PATTERN = re.compile(
    r"\b(?:retriev(?:e|es|ed|ing)|pick(?:s|ed)?\s+up|take(?:s|n)?|grab(?:s|bed)?)\b",
    re.IGNORECASE,
)
_FIRE_PATTERN = re.compile(
    r"\b(?:fire(?:s|d|ing)?|shoot(?:s|ing)?|uses?|aims?)\b",
    re.IGNORECASE,
)
_DROP_PATTERN = re.compile(r"\b(?:drop(?:s|ped|ping)?|lets?\s+go\s+of|abandons?)\b", re.IGNORECASE)
_LOCK_PATTERN = re.compile(r"\b(?:lock(?:s|ed|ing)?|bolt(?:s|ed|ing)?)\b", re.IGNORECASE)
_EQUIP_PATTERN = re.compile(
    r"\b(?:equip(?:s|ped|ping)?|holster(?:s|ed|ing)?|"
    r"strap(?:s|ped|ping)?(?:\s+\S+){0,6}\s+(?:to|onto|around)|"
    r"sling(?:s|ed|ing)?(?:\s+\S+){0,6}\s+(?:over|across|onto)|"
    r"puts?\s+on)\b",
    re.IGNORECASE,
)

# Planning annotations are internal bookkeeping and must not reach beats.txt.
# Keep these patterns deliberately narrow so ordinary story prose is preserved.
...
_UNFINISHED_ACTION_PATTERN = re.compile(
    r"(?i)\b(?:begins?|starts?|tries?|attempts?|partway|midway|in\s+the\s+"
    r"process\s+of|not\s+yet\s+(?:complete|finished))\b"
)

_COMPLETION_ACTION_PATTERN = re.compile(
    r"(?i)\b(?:finishes?|completes?|completely|finally|successfully|stands?|"
    r"steps?\s+(?:inside|out)|locks?|opens?|enters?|exits?|retrieves?|"
    r"raises?|fires?|drops?|grabs?|takes?)\b"
)
...
def configure_formatter(model):
    ...
    global ACTIVE_FORMATTER, format_mistral_prompt, validate_mistral_prompt
...
def _guard_combined_continuity_subjects(
    ...
    for raw_name, record in candidate_map.items():
        ...
        for field, value in record.items():
            if field in SUBJECT_CANONICAL_STATE_FIELDS:
                target[field] = copy.deepcopy(value)
    allowed_top_level = set(CONTINUITY_TOP_LEVEL_FIELDS)
    guarded = {
...
# Extract the final-frame boundary written by Director Request 1.
def _extract_end_continuity_state(raw_scene):
    """Return Request 1's trailing End continuity state without rewriting it."""
...
def _prune_phase2_continuity_placeholders(value):
    """Return a Phase-2-only copy without unknown or empty placeholders."""
    def prune(item):
        if item is None:
            return _PHASE2_OMITTED
        if isinstance(item, str):
            return (
                _PHASE2_OMITTED
                if not item.strip() or item.strip().upper() == "N/A"
                else item
            )
        if isinstance(item, dict):
            ...
            for key, child in item.items():
                pruned = prune(child)
                if pruned is not _PHASE2_OMITTED:
                    cleaned[key] = pruned
            return cleaned if cleaned else _PHASE2_OMITTED
    ...
    pruned = prune(value)
    return {} if pruned is _PHASE2_OMITTED else pruned
...
def _phase2_continuity_state_for_scene(
    ...
    scene = _continuity_scene_description(
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            if (
                ...
                # Without response_format, Mistral sometimes adds this
...
# Return a fresh concise canonical state for story-beat validation.
def new_beat_canonical_state():
    return {
        "version": 1,
        "characters": {},
        "environment": {
...
def normalize_beat_canonical_state(state):
    if not isinstance(state, dict):
    ...
    if unknown_fields:
        raise ValueError(
            "Canonical beat state has unsupported top-level fields: "
            + ", ".join(sorted(unknown_fields))
        )
    normalized = new_beat_canonical_state()
    ...
    if (
        ...
        raise ValueError("Canonical beat state has an unsupported version.")

    characters = state.get("characters", {})
    if not isinstance(characters, dict):
        raise ValueError("Canonical beat state characters must be an object.")
    for character_id, raw_record in characters.items():
        ...
        record.setdefault("weapon_state", "N/A")
        record.setdefault("injuries", [])
        record.setdefault("body_condition", "N/A")
        record.setdefault("status", "alive/conscious unless stated otherwise")
        record.setdefault("current_action", "N/A")
        record.setdefault("relationships", [])
        record["clothing"] = _normalize_clothing(record["clothing"])
        for field in (
            "held_objects",
            "equipped_objects",
            "stored_objects",
    ...
    environment = state.get("environment", {})
    if not isinstance(environment, dict):
        raise ValueError("Canonical beat state environment must be an object.")
    normalized["environment"].update(copy.deepcopy(environment))
    for field in ("doors", "windows", "barriers", "objects", "paths"):
        if not isinstance(normalized["environment"].get(field), dict):
            raise ValueError(f"Canonical environment {field} must be an object.")
    for field in ("persistent_effects", "hazards"):
        normalized["environment"][field] = _state_string_list(
            normalized["environment"].get(field)
        )

    threats = state.get("threats", {})
    if not isinstance(threats, dict):
        raise ValueError("Canonical beat state threats must be an object.")
    for threat_id, raw_record in threats.items():
        if not isinstance(threat_id, str) or not threat_id.strip():
            raise ValueError("Canonical beat state contains an invalid threat ID.")
        if not isinstance(raw_record, dict):
            raise ValueError(f"Threat {threat_id!r} state must be an object.")
        record = copy.deepcopy(raw_record)
        record.setdefault("location", "N/A")
        record.setdefault("status", "alive unless stated otherwise")
        record.setdefault("injuries", [])
        record.setdefault("body_topology", "N/A")
        record.setdefault("current_action", "N/A")
        record.setdefault("has_exited", False)
        record["injuries"] = _state_string_list(record["injuries"])
        if isinstance(record["has_exited"], str):
            lowered = record["has_exited"].strip().casefold()
            if lowered in {"true", "false"}:
                record["has_exited"] = lowered == "true"
        if not isinstance(record["has_exited"], bool):
    ...
    normalized["story_progress"].update(copy.deepcopy(progress))
    for field in ("completed_required_event_ids", "pending_required_event_ids"):
        normalized["story_progress"][field] = _state_string_list(
            normalized["story_progress"].get(field)
        )
...
def _threat_number(threat_id):
    match = _THREAT_ID_PATTERN.fullmatch(str(threat_id or "").strip())
...
def _normalize_threat_patch_ids(current_state, patch, next_threat_number_hint=None):
    """Assign stable IDs to new threat patch entries without renaming history."""
    normalized = copy.deepcopy(patch)
    threats = normalized.get("threats")
    if not isinstance(threats, dict):
        return normalized
    existing = set((current_state or {}).get("threats", {}))
    assigned = set(existing)
    highest_existing = max(
        [(_threat_number(key) or 0) for key in assigned] or [0]
    )
    next_number = max(highest_existing + 1, int(next_threat_number_hint or 1))
    for raw_id, record in list(threats.items()):
        if raw_id in existing:
            assigned.add(raw_id)
            continue
        # Keep already-numbered legacy IDs stable for old checkpoints. New
        # anonymous or model-invented labels, including threat_99, receive
        # Python-owned canonical IDs.
        target_id = f"threat_{next_number}"
        while target_id in assigned:
            next_number += 1
            target_id = f"threat_{next_number}"
        next_number += 1
        assigned.add(target_id)
        threats[target_id] = threats.pop(raw_id)
    return normalized
...
def _validate_state_patch_key(key):
    """Validate one structural JSON key without interpreting it as a path."""
    if not isinstance(key, str) or not key.strip() or key != key.strip():
        raise ValueError("State patches must contain non-empty string keys.")
    if key.startswith("_") or key in {".", ".."}:
        raise ValueError(f"State patch contains an invalid key: {key!r}.")


def _validate_state_patch_value(value, path=()):
    """Validate a JSON-shaped patch while allowing flexible leaf fields."""
    if value is None or isinstance(value, (str, int, float, bool)):
        return
    if isinstance(value, list):
        for index, item in enumerate(value):
            _validate_state_patch_value(item, path + (str(index),))
        return
    if isinstance(value, dict):
        for key, item in value.items():
            _validate_state_patch_key(key)
            _validate_state_patch_value(item, path + (key,))
        return
    location = ".".join(path) or "patch"
    raise ValueError(f"State patch contains a non-JSON value at {location!r}.")


def normalize_beat_state_patch(patch):
    """Validate and copy a partial canonical-state patch.

    The patch deliberately validates structure rather than a hard-coded set of
    entity leaves. Python owns normalization of known fields after merging, so
    harmless persistent fields can pass through unchanged.
    """
    if not isinstance(patch, dict):
        raise ValueError("Beat state patch must be an object.")
    unknown_roots = set(patch) - _BEAT_STATE_ROOTS
    if unknown_roots:
        raise ValueError(
            "Beat state patch has unsupported top-level roots: "
            + ", ".join(sorted(unknown_roots))
        )
    for root, value in patch.items():
        if not isinstance(value, dict):
            raise ValueError(f"Beat state patch root {root!r} must be an object.")
        if root in _BEAT_ENTITY_ROOTS:
            for entity_id, entity_patch in value.items():
                _validate_state_patch_key(entity_id)
                if not isinstance(entity_patch, dict):
                    raise ValueError(
                        f"Beat state patch entity {root}.{entity_id} must be an object."
                    )
...
def _deep_merge_state(current, patch):
    """Recursively merge a validated patch without mutating either input."""
    ...
    for key, value in patch.items():
        if (
            ...
            # None is the canonical clear/reset value. Known collection fields
...
def _entity_status_is_irreversible(status):
    """Return whether a status is explicitly terminal/non-reversible."""
    lowered = str(status or "").casefold()
    return any(
        marker in lowered
        for marker in (
            "inactive", "destroyed", "exited", "trapped", "incapacitated",
            "resolved", "removed", "finished", "complete",
        )
    )

def _enforce_irreversible_entity_statuses(previous, state):
    """Prevent an unauthorized partial patch from reversing terminal entity state."""
...
def _state_object_entries(state):
    """Yield normalized object names and their canonical ownership records."""
    entries = {}

    def add(name, owner, field, value):
        key = " ".join(str(name or "").split()).strip().casefold()
        if key:
            entries.setdefault(key, []).append((owner, field, value))
    ...
    for character_id, record in (state or {}).get("characters", {}).items():
        ...
        for field in ("held_objects", "equipped_objects", "stored_objects"):
            for value in record.get(field, []) or []:
                add(value, character_id, field, record)
        for weapon_name, weapon_state in (record.get("weapons") or {}).items():
            add(weapon_name, character_id, "weapons", weapon_state)
    environment = (state or {}).get("environment", {})
    for object_name, object_state in (environment.get("objects") or {}).items():
        add(object_name, "environment", "objects", object_state)
    return entries


def _object_name_in_text(text, object_name):
    return str(object_name).casefold() in str(text or "").casefold()


def _record_text(record):
    return json.dumps(record or {}, ensure_ascii=False).casefold()


def _record_values_text(record):
    """Serialize only state values, avoiding false matches from field names."""
    if isinstance(record, dict):
        return " ".join(_record_values_text(value) for value in record.values()).casefold()
    if isinstance(record, list):
        return " ".join(_record_values_text(value) for value in record).casefold()
...
def _active_threat(record):
    ...
    return not any(
        marker in status
        for marker in (
            "inactive", "destroyed", "incapacitated", "resolved", "removed",
            "exited", "finished", "complete",
        )
    )

def sanitize_beat_planning_metadata(beat_text):
    """Strip only obvious trailing planning annotations from one beat."""
    cleaned = " ".join(str(beat_text or "").split()).strip()
    removed = []
    changed = True
...
def validate_beat_planning_metadata(beat_text):
...
def _event_words(text):
    stopwords = {
        "a", "an", "and", "at", "behind", "by", "from", "in", "into",
        "is", "of", "on", "the", "then", "to", "with", "their", "them",
        "they", "this", "that", "event", "character", "person",
    }
    return [
        word for word in re.findall(r"[a-z0-9]+", str(text or "").casefold())
        ...
        for event in phase.get("required_events", []):
            if isinstance(event, dict) and event.get("id") and event.get("event"):
                record = {
                    ...
                    # Keep the Python-owned global assignment with the
                    # normalized record.  The forward coordinator uses this
                    # same record for both job selection and completion.
...
def _ordered_required_event_ids(all_event_ids, completed_ids):
    """Return completed required-event IDs in canonical arc order."""
    completed_keys = {
        str(event_id).casefold() for event_id in (completed_ids or ())
    }
    return [
        event_id for event_id in all_event_ids
        if str(event_id).casefold() in completed_keys
    ]
    ...
    for phase in (macro_arc or {}).get("phases", []):
        ...
        for event in phase.get("required_events", []):
            if isinstance(event, dict) and event.get("id"):
                mapping[str(event["id"])] = phase_number
    return mapping
...
def _python_owned_state_patch(patch):
    ...
    progress = patch.get("story_progress")
    if not isinstance(progress, dict):
        return patch
    for field in (
        "current_macro_phase",
...
def validate_completed_event_ids(
    ...
    """Return valid new event IDs and diagnostics for invalid model bookkeeping."""
...
def _phase_records_for_window(macro_arc, start, end):
    return [
        phase for phase in (macro_arc or {}).get("phases", [])
        if isinstance(phase, dict)
        and phase.get("beat_end", 0) >= start
        and phase.get("beat_start", 0) <= end
    ]
...
BEAT_VALIDATION_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
        "name": "story_beat_validation",
        "strict": True,
        "schema": {
            "type": "object",
            "properties": {
...
def _connective_beat_job():
    ...
    return (
        "Advance the current macro phase with one concise connective beat that is "
        "consistent with canonical state and does not complete a later required event early."
    )
...
def parse_beat_validation_result(raw_result):
    ...
    if isinstance(result, str):
        # Beat validation has a deliberately small contract.  In particular,
        # malformed validator output must consume the normal retry budget rather
        # than starting a separate JSON-repair request.
        result = json.loads(result.strip())
    if not isinstance(result, dict) or set(result) != {"valid", "issue"}:
        raise ValueError("Beat validation must contain only valid and issue.")
    if not isinstance(result["valid"], bool):
        raise ValueError("Beat validation returned a non-boolean valid field.")
...
by this beat):
{
  ...
  "environment": {
    "doors": {"storage door": {"status": "locked"}}
  }
}
...
def beat_validation_fingerprint(
    story,
    total_segments,
    macro_arc,
    beat_instructions="",
    ...
    payload = {
        "story": str(story or ""),
        "total_segments": int(total_segments),
        "macro_arc": macro_arc or {},
        "beat_instructions": str(beat_instructions or ""),
        "subject_information": str(subject_information or ""),
        "phrase_exclusions": list(phrase_exclusions or ()),
    }
...
def get_beat_validation_state_path(beats_path=BEATS_FILE, state_path=None):
    if state_path is not None:
        return os.path.abspath(os.fspath(state_path))
    return os.path.join(os.path.dirname(os.path.abspath(os.fspath(beats_path))), "beat_validation_state.json")


def save_beat_validation_state(state, path):
    directory = os.path.dirname(os.path.abspath(path))
    os.makedirs(directory, exist_ok=True)
    descriptor, temporary_path = tempfile.mkstemp(prefix=".beat_validation_", suffix=".tmp", dir=directory, text=True)
    try:
...
def load_beat_validation_state(path):
    try:
...
def _beat_validation_state_from_checkpoint(
    checkpoint,
    fingerprint,
    total_segments,
    macro_arc=None,
    ...
    if not checkpoint or checkpoint.get("fingerprint") != fingerprint:
        return None
    framework = checkpoint.get("framework_beats")
    finalized = checkpoint.get("finalized_beats")
    if not isinstance(framework, list) or len(framework) != int(total_segments):
        raise ValueError("Beat validation checkpoint has an invalid framework.")
    if not isinstance(finalized, list):
        raise ValueError("Beat validation checkpoint has no finalized beat list.")
    finalized_texts = []
    for expected_number, record in enumerate(finalized, start=1):
        if not isinstance(record, dict) or set(record) != {"beat_number", "beat_text"}:
            raise ValueError("Beat validation checkpoint has an invalid finalized beat.")
        if record["beat_number"] != expected_number or not str(record["beat_text"]).strip():
            raise ValueError("Beat validation checkpoint finalized beats are out of order.")
        finalized_texts.append(str(record["beat_text"]).strip())
    finalized_through = checkpoint.get("finalized_through", len(finalized_texts))
    if finalized_through != len(finalized_texts) or finalized_through > int(total_segments):
        raise ValueError("Beat validation checkpoint has an invalid finalized prefix.")
    checkpoint["framework_beats"] = [str(beat) for beat in framework]
    checkpoint["finalized_beats"] = [
        {"beat_number": number, "beat_text": text}
        for number, text in enumerate(finalized_texts, start=1)
    ]
    checkpoint["beat_state_after"] = {
        str(number): normalize_beat_canonical_state(state)
        for number, state in (checkpoint.get("beat_state_after") or {}).items()
    }
    for number in range(1, finalized_through + 1):
        if str(number) not in checkpoint["beat_state_after"]:
            raise ValueError("Beat validation checkpoint is missing a state snapshot.")
    checkpoint["current_beat_state"] = normalize_beat_canonical_state(
        checkpoint.get("current_beat_state") or new_beat_canonical_state()
    )
    stored_next_id = checkpoint.get("next_threat_id")
    if (
        ...
        stored_next_id = max(
            [
                (_threat_number(threat_id) or 0) + 1
                for snapshot in checkpoint["beat_state_after"].values()
                for threat_id in snapshot.get("threats", {})
            ]
            or [1]
        )
    checkpoint["next_threat_id"] = stored_next_id
    for phase in (macro_arc or {}).get("phases", []):
        phase_end = phase.get("beat_end") if isinstance(phase, dict) else None
        if not isinstance(phase_end, int) or phase_end > finalized_through:
            continue
        boundary_state = checkpoint["beat_state_after"].get(str(phase_end))
        completed = set(
            boundary_state.get("story_progress", {}).get(
                "completed_required_event_ids", []
            )
        )
        missing_events = [
            event.get("id") for event in phase.get("required_events", [])
            if isinstance(event, dict) and event.get("id") not in completed
        ]
        if missing_events:
            details = []
            if missing_events:
...
def _truncate_beat_validation_checkpoint_to_phase_prefix(
    checkpoint,
    prefix_end,
    framework_beats,
    macro_arc,
):
    """Discard a failed phase and every later phase from a beat checkpoint."""
    prefix_end = int(prefix_end)
    if prefix_end < 0:
        raise ValueError("Beat validation phase prefix cannot be negative.")
    if not isinstance(framework_beats, list):
        raise ValueError("Beat validation phase retry requires a beat framework.")

    finalized = checkpoint.get("finalized_beats") or []
    if len(finalized) < prefix_end:
        raise ValueError(
            "Beat validation checkpoint does not contain the finalized phase prefix."
        )
    finalized = finalized[:prefix_end]
    snapshots = checkpoint.get("beat_state_after") or {}
    if prefix_end:
        if str(prefix_end) not in snapshots:
            raise ValueError(
                "Beat validation checkpoint is missing the phase-prefix state snapshot."
            )
        current_state = normalize_beat_canonical_state(snapshots[str(prefix_end)])
    else:
        current_state = new_beat_canonical_state()

    all_events = _required_event_records(macro_arc)
    all_event_ids = [event["id"] for event in all_events]
    completed = set(
        current_state.get("story_progress", {}).get(
            "completed_required_event_ids", []
        )
    )
    completed_keys = {str(event_id).casefold() for event_id in completed}
    current_state["story_progress"]["completed_required_event_ids"] = (
        _ordered_required_event_ids(all_event_ids, completed)
    )
    current_state["story_progress"]["pending_required_event_ids"] = [
        event_id for event_id in all_event_ids
        if str(event_id).casefold() not in completed_keys
    ]

    checkpoint["framework_beats"] = list(framework_beats)
    checkpoint["finalized_beats"] = [
        {"beat_number": index, "beat_text": record["beat_text"]}
        for index, record in enumerate(finalized, start=1)
    ]
    checkpoint["beat_state_after"] = {
        str(index): normalize_beat_canonical_state(snapshots[str(index)])
        for index in range(1, prefix_end + 1)
    }
    checkpoint["current_beat_state"] = current_state
    checkpoint["completed_required_event_ids"] = (
        _ordered_required_event_ids(all_event_ids, completed)
    )
    checkpoint["pending_required_event_ids"] = [
        event_id for event_id in all_event_ids
        if str(event_id).casefold() not in completed_keys
    ]
    checkpoint["finalized_through"] = prefix_end
    checkpoint["next_window_start"] = (
        None if prefix_end >= len(framework_beats) else prefix_end + 1
    )
    checkpoint["next_threat_id"] = max(
        [
            (_threat_number(threat_id) or 0) + 1
            for threat_id in current_state.get("threats", {})
        ]
        or [1]
    )
    return checkpoint
...
class BeatValidationExhaustedError(ValueError):
    ...
    def __init__(self, beat_number, message=None, phase_number=None, scope="beat"):
        self.beat_number = int(beat_number)
        self.phase_number = (
            int(phase_number) if phase_number is not None else None
        )
        self.scope = str(scope or "beat")
        super().__init__(
            message
...
# Single-candidate beat validation replaces the former local fidelity,
# continuity, state-extraction, and repair pipeline. Keep each validator call
# immutable: it may reject a candidate, but it can never return replacement
# beat text.
def _run_forward_beat_validation(
    ...
    checkpoint = None if reset_state else load_beat_validation_state(state_path)
    checkpoint = _beat_validation_state_from_checkpoint(
        checkpoint, fingerprint, total_segments, macro_arc=macro_arc
    )

    if checkpoint is None:
        framework = parse_generated_beats(
            {"beats": framework_factory()},
            int(total_segments),
            phrase_exclusions=phrase_exclusions,
        )
        instruction_issues = validate_generated_beat_instructions(
            framework, beat_instructions
        )
        if instruction_issues:
            raise ValueError(" ".join(instruction_issues))
        all_event_ids = [event["id"] for event in _required_event_records(macro_arc)]
        checkpoint = {
            "version": BEAT_VALIDATION_STATE_VERSION,
            "fingerprint": fingerprint,
            "total_beats": int(total_segments),
            "framework_beats": list(framework),
            "finalized_beats": [],
            "beat_state_after": {},
            "current_beat_state": new_beat_canonical_state(),
            "completed_required_event_ids": [],
            "pending_required_event_ids": all_event_ids,
            "next_threat_id": 1,
            "finalized_through": 0,
            "next_window_start": 1,
        }
        save_beat_validation_state(checkpoint, state_path)
    else:
        framework = list(checkpoint["framework_beats"])

    finalized_texts = [record["beat_text"] for record in checkpoint["finalized_beats"]]
    finalized_through = int(checkpoint["finalized_through"])
    all_events = _required_event_records(macro_arc)
    events_by_id = {event["id"]: event for event in all_events}
    all_event_ids = [event["id"] for event in all_events]
    completed_cursor = set(checkpoint.get("completed_required_event_ids", []))
    state_cursor = normalize_beat_canonical_state(
        checkpoint.get("current_beat_state") or new_beat_canonical_state()
    )
    state_cursor = _seed_known_beat_characters(
        state_cursor,
        macro_arc=macro_arc,
        subject_information=subject_information,
    )
    completed_keys = {
        str(event_id).casefold() for event_id in completed_cursor
    }
    state_cursor["story_progress"]["completed_required_event_ids"] = (
        _ordered_required_event_ids(all_event_ids, completed_cursor)
    )
    ...
    def save_checkpoint():
        checkpoint["framework_beats"] = list(framework)
        checkpoint["finalized_beats"] = [
            {"beat_number": number, "beat_text": text}
            for number, text in enumerate(finalized_texts, start=1)
        ]
        ...
        checkpoint["completed_required_event_ids"] = _ordered_required_event_ids(
            all_event_ids, completed_cursor
        )
        checkpoint["pending_required_event_ids"] = [
            event_id for event_id in all_event_ids
            if str(event_id).casefold() not in {
        ...
        checkpoint["next_threat_id"] = max(
            ...
            max(
                [
                    (_threat_number(threat_id) or 0) + 1
                    for threat_id in state_cursor.get("threats", {})
                ]
                or [1]
            ),
        )
    ...
    # Persist normalized initial state (including seeded known characters and
    # Python-owned required-event collections) before the first validator call.
    # This is initialization, not candidate acceptance, so rejected attempts
    # still leave the authoritative snapshot unchanged.
    save_checkpoint()
    ...
    def job_for_beat(beat_number, planned_candidate, phase):
        assigned = [
            ...
            if event.get("beat_number") == beat_number and event.get("event")
        ]
        if assigned:
            return " ".join(str(event).strip() for event in assigned)
        return _connective_beat_job()

    def next_required_job_after(beat_number):
        later_beats = sorted({
            int(event["beat_number"])
            for event in all_events
            if isinstance(event.get("beat_number"), int)
            and not isinstance(event.get("beat_number"), bool)
            and int(event["beat_number"]) > int(beat_number)
        })
        if not later_beats:
            return None, None
        target_beat = later_beats[0]
        events = [
            event for event in all_events
            if event.get("beat_number") == target_beat and event.get("event")
        ]
        if not events:
            return target_beat, None
        return target_beat, " ".join(str(event["event"]).strip() for event in events)

    def assigned_job_label(beat_number, job):
        assigned = [
            event for event in all_events
            if event.get("beat_number") == beat_number
        ]
        if assigned:
            return "; ".join(
                f"{event['id']} - {event['event']}" for event in assigned
            )
        return job
    ...
    def issue_text(issues):
        ...
        for issue in issues or ():
            if isinstance(issue, dict):
                rendered.append(
                    str(issue.get("problem") or issue.get("issue") or issue)
                )
            else:
                rendered.append(str(issue))
        return "; ".join(item.strip() for item in rendered if item.strip())

    for beat_number in range(finalized_through + 1, int(total_segments) + 1):
        phase = phase_for_beat(beat_number)
        planned_candidate = str(framework[beat_number - 1]).strip()
        assigned_current_events = [
            event for event in all_events
            if event.get("beat_number") == beat_number
        ]
        current_job = job_for_beat(beat_number, planned_candidate, phase)
        next_job_beat, next_job = next_required_job_after(beat_number)

        completed_before = _ordered_required_event_ids(
            all_event_ids, completed_cursor
        )
        pending_before = [
            event_id for event_id in all_event_ids
        ...
        print(
            ...
            f"completed before={', '.join(completed_before) or 'none'}; "
            f"pending before={', '.join(pending_before) or 'none'}",
            flush=True,
        )
        ...
        accepted = False
        last_issue = ""
        for validation_attempt in range(1, BEAT_RETRY_ATTEMPTS + 1):
            if validation_attempt > 1 and regenerate_candidate:
                if candidate_factory is None:
                    ...
                    try:
                        candidate = str(
                            candidate_factory(
                                beat_number=beat_number,
                                previous_finalized_beats=list(finalized_texts),
            ...
            try:
                ...
                structural_issues.append(str(error))

            if structural_issues:
                last_issue = "Beat failed structural validation: " + issue_text(
                    dict.fromkeys(structural_issues)
                )
                retry_feedback = last_issue
        ...
        if not accepted:
            raise BeatValidationExhaustedError(
                beat_number,
                last_issue or "The beat did not pass validation.",
            )

        # Stage every authoritative mutation until the accepted candidate's
        # required-event effects have been applied successfully. Rejected
        # candidates never reach this block, and a commit failure leaves the
        # outer cursors untouched.
        # The production beat validator is the semantic authority. Once it
        # returns VALID, every event explicitly assigned to this beat is
        # complete. Do not re-check the prose or dependencies here: doing so
        # creates a second semantic completion decision after validation.
        completed_keys = {
            str(event_id).casefold() for event_id in completed_cursor
        }
        newly_completed = [
            ...
            for event in all_events
            if event.get("beat_number") == beat_number
            and str(event["id"]).casefold() not in completed_keys
        ]
        staged_completed = set(completed_cursor)
        staged_completed.update(newly_completed)
        ...
        completed_keys = {
            str(event_id).casefold() for event_id in staged_completed
        }
        staged_state["story_progress"]["pending_required_event_ids"] = [
            event_id for event_id in all_event_ids
            if str(event_id).casefold() not in completed_keys
        ]
        if phase:
            staged_state["story_progress"]["current_macro_phase"] = phase.get(
                "phase_number"
            )

        completed_cursor = staged_completed
        state_cursor = normalize_beat_canonical_state(staged_state)

        framework[beat_number - 1] = candidate
        finalized_texts.append(candidate)
        ...
        if phase and beat_number == phase.get("beat_end"):
            missing = [
                event["id"]
                for event in all_events
                if event.get("beat_number") is not None
                and event.get("id") not in completed_cursor
                and (
                    story_arc_phase_for_beat(
                        macro_arc, event["beat_number"]
                    ) or {}
                ).get("phase_number") == phase.get("phase_number")
            ]
            if missing:
                raise BeatValidationExhaustedError(
                    beat_number,
                    "Phase boundary is missing required events: "
                    + ", ".join(missing),
                    phase_number=phase.get("phase_number"),
                    scope="phase",
                )
    ...
    for phase in (macro_arc or {}).get("phases", []):
        phase_end = phase.get("beat_end")
        if not isinstance(phase_end, int) or phase_end > int(total_segments):
            continue
        missing = [
            event["id"]
            for event in phase.get("required_events", [])
            if event.get("id") not in completed_cursor
        ]
        if missing:
            raise BeatValidationExhaustedError(
                phase_end,
                "Phase boundary is missing required events: " + ", ".join(missing),
                phase_number=phase.get("phase_number"),
                scope="phase",
            )
...
def _seed_known_beat_characters(state, macro_arc=None, subject_information=""):
    """Seed known characters with explicit closed-world injury history.

    This is intentionally limited to characters already named by the story
    inputs or macro arc.  It does not infer events or injuries from prose.
    """
    state = normalize_beat_canonical_state(state or new_beat_canonical_state())
    names = []
    seen = set()
    ...
    for subject_line in str(subject_information or "").splitlines():
        subject_line = subject_line.strip().lstrip("- ")
        subject_name, separator, _details = subject_line.partition(" is ")
        if separator:
            add_name(subject_name)
    for phase in (macro_arc or {}).get("phases", []):
        if not isinstance(phase, dict):
            continue
    ...
    existing_by_key = {
        str(character_id).casefold(): character_id
        for character_id in state.get("characters", {})
    }
    for name in names:
        character_id = existing_by_key.get(name.casefold(), name)
        record = state["characters"].setdefault(character_id, {})
        injury_values = (
            record.get("current_injuries"),
            record.get("prior_injuries"),
            record.get("injury_history"),
        )
        no_injury_markers = {"", "none", "n/a", "no injury"}
        has_established_injury = bool(record.get("injuries")) or any(
            " ".join(str(value or "").split()).casefold() not in no_injury_markers
            and "no injury event" not in " ".join(
                str(value or "").split()
            ).casefold()
            for value in injury_values
            if value is not None
        )
        if not has_established_injury:
            record["current_injuries"] = "none"
            record["prior_injuries"] = "none"
            record["injury_history"] = (
                "no injury event has been established at any earlier beat"
            )
...
def build_macro_arc_repair_messages(
    story,
    macro_arc,
    issues,
    total_segments,
    ...
    return [
        {
            "role": "system",
            "content": (
                "You repair a rejected macro story arc. Correct only the issues "
                "listed by the validator while preserving every unaffected phase, "
                "event, assignment, dependency, and state effect. You may change "
                "narrative fields when directly required by an issue. Return one "
                "complete macro arc using the normal phases schema; never return a "
                "patch, coverage metadata, claim IDs, or commentary."
            ),
        },
        {
            "role": "user",
...
def parse_beat_arc_plan(
    ...
    formatter=None,
    llm_request=None,
    validate_state_effects=True,
):
    """Parse and validate a complete macro story arc deterministically."""
    if total_segments <= 0:
    ...
    if not isinstance(candidate, dict) or not isinstance(candidate.get("phases"), list):
    ...
    for phase_number, phase in enumerate(phases, start=1):
        ...
        missing = required_fields - set(phase)
        if missing:
            raise ValueError(
        ...
        if phase["phase_number"] != phase_number or isinstance(
            phase["phase_number"], bool
        ) or not isinstance(phase["phase_number"], int):
            raise ValueError(
                f"Macro arc phase {phase_number} must have phase_number {phase_number}."
            )
        start, end = phase["beat_start"], phase["beat_end"]
        if isinstance(start, bool) or not isinstance(start, int):
            raise ValueError(f"Macro arc phase {phase_number} beat_start must be an integer.")
        if isinstance(end, bool) or not isinstance(end, int):
            raise ValueError(f"Macro arc phase {phase_number} beat_end must be an integer.")
        ...
        characters = [" ".join(character.split()) for character in characters]
        if len({character.casefold() for character in characters}) != len(characters):
            raise ValueError(
        ...
        if not isinstance(events, list):
            raise ValueError(
                f"Macro arc phase {phase_number} required_events must be an array."
            )
        normalized_events = []
        for event_index, event in enumerate(events, start=1):
            if (
                not isinstance(event, dict)
                or not {"id", "event"}.issubset(event)
            ...
            if not isinstance(event["id"], str) or not isinstance(event["event"], str):
                raise ValueError(
                    f"Macro arc phase {phase_number} required event {event_index} "
                    "id and event must be strings."
                )
            event_id = " ".join(event["id"].split()).strip()
            event_text = " ".join(event["event"].split()).strip()
            ...
            if event_key in required_event_ids:
                raise ValueError(f"Duplicate macro required event ID: {event_id}.")
            required_event_ids.add(event_key)
            beat_number = event.get("beat_number")
            ...
            dependencies = [" ".join(dependency.split()).strip() for dependency in dependencies]
            if len({dependency.casefold() for dependency in dependencies}) != len(dependencies):
                raise ValueError(
                    f"Macro arc phase {phase_number} required event {event_index} "
                    "depends_on contains duplicates."
                )
            if event_key in {dependency.casefold() for dependency in dependencies}:
                raise ValueError(
...
def build_phase_generation_batches(macro_arc, max_batch_size=None):
    ...
    for phase_index, phase in enumerate(phases, start=1):
        ...
        start, end = phase.get("beat_start"), phase.get("beat_end")
        if phase.get("phase_number") != phase_index or not isinstance(start, int) or not isinstance(end, int) or start != expected_start or end < start:
            raise ValueError(f"Macro arc phase {phase_index} has an invalid beat range.")
        batch_size = max_batch_size or (end - start + 1)
...
def parse_macro_arc_validation_result(
    ...
    if isinstance(candidate, str):
        ...
        try:
            ...
            raise ValueError(
                "The macro-arc validation response must be valid JSON."
            ) from error
    if not isinstance(candidate, dict):
        raise ValueError("The macro-arc validation response must be a JSON object.")
    valid, issues = candidate.get("valid"), candidate.get("issues")
    if not isinstance(valid, bool):
        raise ValueError("The macro-arc validation 'valid' field must be boolean.")
    if not isinstance(issues, list) or not all(
        isinstance(issue, str) and issue.strip() for issue in issues
    ):
        raise ValueError("The macro-arc validation 'issues' field must be a string array.")
...
def build_macro_arc_validation_response_format():
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "story_macro_arc_validation",
            "strict": True,
            "schema": {
                "type": "object",
                "properties": {
...
def _source_excerpt_for_blocker(issue, story, max_chars=900):
    """Select a small source excerpt relevant to one blocker when possible."""
    if not story:
        return ""
    requirement = str(issue.get("source_requirement", ""))
    tokens = {
        token
        for token in re.findall(r"[a-z0-9]+", requirement.casefold())
        if len(token) > 3 and token not in {"must", "shall", "should", "phase", "beat"}
    }
    if not tokens:
        return ""
    sentences = [part.strip() for part in re.split(r"(?<=[.!?])\s+", str(story)) if part.strip()]
    relevant = [
        sentence
        for sentence in sentences
        if len(tokens & set(re.findall(r"[a-z0-9]+", sentence.casefold()))) >= 1
    ]
    excerpt = " ".join(relevant[:3]).strip()
    return excerpt[:max_chars]


def _compact_blocker_context(issue, issue_id, beats, story="", macro_arc=None):
    issue = dict(issue or {})
    start = max(1, int(issue.get("beat_start", 1)))
    end = min(len(beats), max(start, int(issue.get("beat_end", start))))
    if issue.get("type") == ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE:
        start = max(1, end - 1)
    phase = story_arc_phase_for_beat(macro_arc or {}, start)
    phase_context = ""
    if phase:
    ...
    source_excerpt = ""
    if issue.get("type") != ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE:
        source_excerpt = _source_excerpt_for_blocker(issue, story)
    lines = [
        f"ISSUE {issue_id}",
        f"TYPE: {issue.get('type', 'N/A')}",
        f"PROBLEM: {issue.get('problem', 'N/A')}",
        f"RELEVANT BEATS:\n{_numbered_beat_range(beats, start, end) if beats else 'N/A'}",
    ]
    if issue.get("type") == ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE:
        for field in (
            "end_state_before",
            ...
            if issue.get(field):
                lines.append(f"{field.upper()}: {issue[field]}")
    if (
        issue.get("source_requirement")
        and issue.get("type") != ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE
    ):
        lines.append(f"SOURCE REQUIREMENT: {issue['source_requirement']}")
    if source_excerpt:
        lines.append(f"SOURCE EXCERPT:\n{source_excerpt}")
    if phase_context:
        lines.append(f"APPLICABLE PHASE: {phase_context}")
    return "\n".join(lines)


def build_candidate_blocker_verification_response_format(issue_ids):
    """Build the strict response schema for a verifier batch."""
    if isinstance(issue_ids, int) and not isinstance(issue_ids, bool):
        return {
            "type": "json_schema",
            "json_schema": {
    ...
    issue_ids = [int(issue_id) for issue_id in issue_ids]
    if not issue_ids or len(set(issue_ids)) != len(issue_ids):
        raise ValueError("Candidate verification batches require unique issue IDs.")
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "story_candidate_blocker_verification_batch",
            "strict": True,
            "schema": {
                "type": "object",
                "properties": {
                    "results": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "issue_id": {"type": "integer", "enum": issue_ids},
                                "decision": {"type": "string", "enum": ["BLOCK", "DISCARD"]},
                                "reason": {"type": "string", "minLength": 1},
                            },
...
def build_candidate_blocker_verification_batch_messages(
    candidates,
    beats,
    story="",
    macro_arc=None,
):
    """Build one compact prompt for a small batch of candidate blockers."""
    candidates = [dict(candidate) for candidate in candidates or []]
    if not candidates:
        raise ValueError("Candidate verification batches cannot be empty.")
    contexts = [
        _compact_blocker_context(
            candidate,
            candidate["issue_id"],
            beats,
            story=story,
    ...
    return [
        {
            "role": "system",
            "content": (
                "You verify proposed beat-plan blocking issues. For each issue, "
                "return BLOCK only when the supplied evidence directly establishes "
                "the violation. Do not invent off-screen actions, movements, hidden "
                "causes, room layouts, object handling, entrances, exits, or other "
                "explanations. A supplied physical or persistent state remains true "
                "until a supplied beat explicitly changes it. Physical plausibility "
                "alone does not resolve a missing transition. Return DISCARD only "
                "when the proposed issue depends on an unsupported assumption or "
                "misreads the supplied evidence. Do not find new issues. Return a "
                "JSON object with one result for every requested issue ID."
            ),
        },
        {
            "role": "user",
            "content": (
                "CHECK ONLY THESE PROPOSED ISSUES\n\n"
                + "\n\n".join(contexts)
                + "\n\nReturn exactly one result for each issue ID. Each result must contain "
                "issue_id, decision (BLOCK or DISCARD), and a concise reason."
            ),
        },
    ]


def build_candidate_blocker_verification_batches(
    candidates,
    beats=None,
    story="",
    macro_arc=None,
    token_budget=LLM_INPUT_TOKEN_BUDGET,
):
    """Pack verifier candidates until the configured request budget is reached."""
    if (
        isinstance(token_budget, bool)
        or not isinstance(token_budget, int)
        or token_budget <= 0
    ):
        raise ValueError("Candidate verification token_budget must be positive.")
    ...
    for candidate in candidates:
        ...
        if beats is None:
            ...
            tokens = estimate_message_tokens(
                build_candidate_blocker_verification_batch_messages(
                    proposed,
                    beats,
                    story=story,
                    macro_arc=macro_arc,
        ...
        if current and tokens > token_budget:
...
def parse_candidate_blocker_verification_batch(
    raw_result,
    issue_ids,
    formatter=None,
    llm_request=None,
):
    """Parse a batch and require every requested ID exactly once."""
    formatter = formatter or ACTIVE_FORMATTER
    expected = [int(issue_id) for issue_id in issue_ids]
    ...
    if isinstance(candidate, str):
        ...
        try:
            ...
            raise ValueError("The blocker verification batch must be valid JSON.") from error
    if not isinstance(candidate, dict) or set(candidate) != {"results"}:
        raise ValueError("The blocker verification batch must contain only results.")
    results = candidate["results"]
    if not isinstance(results, list):
        raise ValueError("The blocker verification results must be an array.")
    normalized = []
    seen = set()
    for result in results:
        if not isinstance(result, dict) or set(result) != {"issue_id", "decision", "reason"}:
            raise ValueError("Each blocker verification result has invalid fields.")
        issue_id = result["issue_id"]
        if isinstance(issue_id, bool) or not isinstance(issue_id, int):
            raise ValueError("The blocker verification issue ID must be an integer.")
        if issue_id not in expected_set:
            raise ValueError(f"The blocker verifier returned unknown issue ID {issue_id}.")
        if issue_id in seen:
            raise ValueError(f"The blocker verifier repeated issue ID {issue_id}.")
        if result["decision"] not in {"BLOCK", "DISCARD"}:
            raise ValueError("The blocker verifier decision must be BLOCK or DISCARD.")
    ...
    if seen != expected_set or len(normalized) != len(expected):
        missing = sorted(expected_set - seen)
        raise ValueError(
            "The blocker verifier did not return every requested issue ID: "
            + ", ".join(str(issue_id) for issue_id in missing)
        )
    return {
        "results": sorted(
            normalized,
            key=lambda result: expected.index(result["issue_id"]),
        )
    }


def issue_can_bypass_verification(issue, deterministic_proof=None):
    """Return whether Python supplied a direct structural proof for an issue."""
    if not isinstance(issue, dict):
        return False
    if deterministic_proof is not None:
        return deterministic_proof in DETERMINISTIC_VERIFIER_BYPASS_PROOFS
    issue_type = issue.get("type")
    if issue_type == "invalid_beat_range":
    ...
    if issue_type == "nonexistent_phase":
        known_phase_numbers = issue.get("known_phase_numbers")
        return (
            isinstance(known_phase_numbers, (list, tuple, set, frozenset))
            and issue.get("phase_number") not in set(known_phase_numbers)
        )
    if issue_type == "duplicate_beat_id":
        beat_ids = issue.get("beat_ids")
        try:
            return isinstance(beat_ids, list) and len(beat_ids) != len(set(beat_ids))
        except TypeError:
            return False
    if issue_type == "required_count_mismatch":
        expected, actual = issue.get("expected_count"), issue.get("actual_count")
        return (
    ...
    if issue_type == "malformed_structure":
        return bool(str(issue.get("structural_proof", "")).strip())
    return False
...
def build_adjacent_continuity_audit_messages(beats):
    if not beats:
        raise ValueError("Adjacent continuity audit requires at least one beat.")
    return [
        {"role": "system", "content": (
        ...
        {"role": "user", "content": f"""
Audit every adjacent Beat N -> Beat N+1 pair in the complete plan. Flag the
later beat only when it assumes a physical state that Beat N did not establish
and Beat N+1 does not explicitly create.

Check only location, entry/exit, doors and barriers, held or possessed objects,
important spatial relationships, persistent physical condition, and completed
versus unfinished actions. Beat N+1 may explicitly perform the transition.
An unfinished action may continue normally when explicitly unfinished. Explicitly
unfinished movement may continue normally. Do not invent off-screen
transitions. Do not judge source fidelity, story quality, pacing, style,
dialogue, or camera work.

For every definite issue return beat (the later beat number),
end_state_before, opening_state_after, and missing_transition.
...
def parse_adjacent_continuity_audit(raw_result, total_segments, formatter=None, llm_request=None):
    formatter = formatter or ACTIVE_FORMATTER
    candidate = raw_result
    if isinstance(candidate, str):
        candidate = formatter.sanitize_generated_text(candidate)
    ...
    for issue in candidate["issues"]:
        ...
        if not isinstance(issue, dict) or set(issue) != fields:
            raise ValueError("Each adjacent continuity issue has invalid fields.")
        target = issue["beat"]
        if (
            isinstance(target, bool)
            or not isinstance(target, int)
            or target < 2
            or target > int(total_segments)
        ):
            raise ValueError("Adjacent continuity issue beat is outside the plan.")
        if any(
            not isinstance(issue[field], str) or not issue[field].strip()
            for field in fields - {"beat"}
        ):
            raise ValueError("Adjacent continuity issue evidence must be non-empty.")
        normalized.append({
            "beat_start": target, "beat_end": target,
            "type": ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE,
            "source_requirement": "Beat N+1 must be physically reachable from Beat N without an unshown transition.",
            "problem": " ".join(issue["missing_transition"].split()),
            "end_state_before": " ".join(issue["end_state_before"].split()),
            "opening_state_after": " ".join(issue["opening_state_after"].split()),
            "missing_transition": " ".join(issue["missing_transition"].split()),
        })
...
def build_global_fidelity_audit_messages(story, beats, macro_arc, beat_instructions=""):
    return [
        {"role": "system", "content": (
            "You are a conservative story-beat fidelity auditor. Check only "
            "required events, source order, prerequisites, definitive persistent "
            "story states, phase required_end_state, and major concrete source "
            "contradictions. Do not audit adjacent physical movement; another "
            "auditor handles it. Flag only definite failures. Return a JSON object."
        )},
        {"role": "user", "content": f"""
Audit this complete {len(beats)}-beat plan.

For each macro phase:
1. Confirm every required_event occurs visibly in that phase.
2. Confirm required_events occur in listed source order.
3. Confirm required props, states, and results are not used before they are
   established.
4. Confirm definitive persistent states are not later contradicted without an
   explicit authorized event.
5. Confirm required_end_state is true by the phase's final beat.

Also flag a major concrete event that clearly contradicts SOURCE STORY. Do not
judge style, pacing, camera work, exact choreography, ordinary connective
movement, or optional detail. Do not audit ordinary adjacent movement. Do not
re-evaluate macro-arc fidelity or suggest
repair strategy. Explicitly test every phase required_end_state against the
beats through that phase's final beat. Flag only definite failures.
...
MACRO STORY ARC WITH REQUIRED EVENTS
{json.dumps(macro_arc, ensure_ascii=False, indent=2)}
...
with beat_start, beat_end, type, requirement, and problem. Prefer direct
required-event references such as "E4: the operator retrieves the scanner."
""".strip()},
    ]
...
def parse_global_fidelity_audit(raw_result, total_segments, formatter=None, llm_request=None):
    ...
    normalized = []
    fields = {"beat_start", "beat_end", "type", "requirement", "problem"}
    for issue in candidate["issues"]:
        if not isinstance(issue, dict) or set(issue) != fields:
        ...
        if any(not isinstance(issue[field], str) or not issue[field].strip() for field in ("requirement", "problem")):
            raise ValueError("Global fidelity issue text must be non-empty.")
        normalized.append({
            "beat_start": start, "beat_end": end, "type": issue["type"],
            "source_requirement": " ".join(issue["requirement"].split()),
...
def normalize_beat_plan_repair_ranges(issues, total_segments):
    """Return deterministic repair ranges without widening a reported blocker.

    Adjacent physical-transition failures belong to the later beat: repairing
    the earlier beat would change an already-established ending state.  Other
    semantic failures retain the range reported by the auditor.  The helper is
    deliberately conservative because repair orchestration must never invent a
    broader regeneration scope than the validator requested.
    """
    total_segments = int(total_segments)
    normalized_issues = [dict(issue) for issue in (issues or []) if isinstance(issue, dict)]
    ranges = []
    for issue in normalized_issues:
        start = issue.get("beat_start")
        end = issue.get("beat_end")
        if (isinstance(start, bool) or not isinstance(start, int) or
                isinstance(end, bool) or not isinstance(end, int) or
                start <= 0 or end < start or end > total_segments):
            continue
        range_issue = dict(issue)
        if issue.get("type") == ADJACENT_PHYSICAL_TRANSITION_ISSUE_TYPE:
            range_issue["beat_start"] = end
            range_issue["beat_end"] = end
        ranges.append(range_issue)
    ranges.sort(key=lambda issue: (
        issue["beat_start"], issue["beat_end"], str(issue.get("type", ""))))
    return {
...
def targeted_repair_improved(previous_issues, current_issues, _macro_arc=None):
    """Allow targeted repair to continue only when blockers strictly decrease."""
    return len(current_issues or []) < len(previous_issues or [])


def _blocker_phase_numbers(blockers, macro_arc):
    phases = macro_arc.get("phases", []) if isinstance(macro_arc, dict) else []
    matched = set()
    for issue in blockers or []:
        if not isinstance(issue, dict):
            continue
        try:
            start, end = int(issue["beat_start"]), int(issue["beat_end"])
        ...
        for index, phase in enumerate(phases, start=1):
            if not isinstance(phase, dict):
                continue
            try:
                phase_start = int(phase["beat_start"])
                phase_end = int(phase["beat_end"])
            except (KeyError, TypeError, ValueError):
                continue
            if end >= phase_start and start <= phase_end:
                matched.add(phase.get("phase_number", index))
    return sorted(matched, key=lambda value: (value is None, value))


def classify_verified_blocker_scope(blockers, macro_arc):
    """Choose the smallest safe recovery scope for verified beat blockers."""
    blockers = list(blockers or [])
    phase_numbers = _blocker_phase_numbers(blockers, macro_arc)
    if not blockers:
        return {"action": "accept", "phase_number": None}
    if len(phase_numbers) != 1:
        return {"action": "regenerate_phases", "phase_numbers": phase_numbers}

    phase_number = phase_numbers[0]
    phase = next(
        (item for index, item in enumerate(macro_arc.get("phases", []), start=1)
         if isinstance(item, dict) and item.get("phase_number", index) == phase_number),
        {},
    )
    try:
        phase_start, phase_end = int(phase["beat_start"]), int(phase["beat_end"])
    except (KeyError, TypeError, ValueError):
        phase_start = phase_end = None
    wide = any(
        phase_start is not None and
        int(issue.get("beat_start", 0)) <= phase_start and
        int(issue.get("beat_end", 0)) >= phase_end
        for issue in blockers if isinstance(issue, dict)
    )
    action = "regenerate_phase" if wide or len(blockers) >= 3 else "targeted_repair"
    return {"action": action, "phase_number": phase_number}


def classify_verified_blocker_scopes(blockers, macro_arc):
    """Classify each affected phase independently for multi-phase recovery."""
    phases = _blocker_phase_numbers(blockers, macro_arc)
    decisions = []
    for phase_number in phases:
        phase_blockers = []
        for issue in blockers or []:
            if phase_number in _blocker_phase_numbers([issue], macro_arc):
                phase_blockers.append(issue)
        decision = classify_verified_blocker_scope(phase_blockers, macro_arc)
        decisions.append({"phase_number": phase_number, "action": decision["action"]})
    return {"action": "recover_phases", "decisions": decisions}


def classify_regeneration_scope(blockers, macro_arc):
    """Force a phase-level regeneration for callers requiring full phase repair."""
    phases = _blocker_phase_numbers(blockers, macro_arc)
    if len(phases) == 1:
        return {"action": "regenerate_phase", "phase_number": phases[0]}
    return {"action": "regenerate_phases", "phase_numbers": phases}




def blocker_scope_snapshot(blockers, macro_arc):
    """Return deterministic count, phase, and beat-range data for blockers."""
    blockers = list(blockers or [])
    phases = set()
    ranges = []
    macro_phases = (
        macro_arc.get("phases", [])
    ...
    for issue in blockers:
        if not isinstance(issue, dict):
            continue
        try:
            start = int(issue["beat_start"])
            end = int(issue["beat_end"])
        except (KeyError, TypeError, ValueError):
            continue
        ranges.append((start, end))
        matched = False
        for index, phase in enumerate(macro_phases, start=1):
            if not isinstance(phase, dict):
                continue
            try:
                phase_start = int(phase["beat_start"])
                ...
                continue
            if end >= phase_start and start <= phase_end:
                phases.add(phase.get("phase_number", index))
                matched = True
        if not matched:
            phases.add(None)
    return {
...
def build_beat_generation_messages(
    ...
    batch_end=None,
    previous_beats=None,
    previous_phase_final_beat=None,
    end_boundary_beat=None,
    macro_arc=None,
    current_phase=None,
    audit_correction="",
    phrase_exclusions=(),
...
def generate_beats_from_story(
    ...
    # existing callers do not break. Framework generation still uses the
    # existing structural retry behavior; semantic validation is performed only
    # by the forward local-window coordinator below.
    del (
        content_attempts,
    ...
    def request_macro_arc(correction="", combined_attempt=1, max_attempts=10):
        ...
        Args:
            ...
            combined_attempt: Current arc-creation attempt
    ...
    # Validate one complete macro arc; returns (validation, success).
    def request_macro_arc_validation(macro_arc, combined_attempt, max_attempts=10):
        """Request the one semantic validation result for a macro arc.
        
        Args:
            macro_arc: The macro arc to validate
            combined_attempt: Current arc-validation attempt
            max_attempts: Maximum attempts for parsing the validation response
            
        Returns:
            (validation_dict, success) tuple. Success is False if max_attempts reached.
        """
        last_error = None
        response_attempt = 0
        while response_attempt < max_attempts:
            ...
            messages = build_macro_arc_validation_messages(
                story,
                macro_arc,
                subject_information=subject_information,
                beat_instructions=beat_instructions,
            ...
            if last_error:
                messages[-1]["content"] += (
                    "\n\nYOUR PREVIOUS ARC VALIDATION RESPONSE WAS STRUCTURALLY INVALID\n"
        ...
        return ({"valid": False, "issues": ["Arc validation parsing failed after max attempts"]}, False)

    def request_kiss_macro_arc(correction="", max_attempts=None):
        """Run one create/validate/repair arc loop with recovery boundaries."""
        process_budget = max(1, int(max_attempts or BEAT_PROCESS_ATTEMPTS))
        last_error = str(correction or "").strip()
        for process_round in range(1, process_budget + 1):
            try:
                macro_arc, created = request_macro_arc(
                    last_error,
            ...
            current_arc = macro_arc
            for validation_round in range(1, BEAT_RETRY_ATTEMPTS + 1):
                try:
                    validation, parsed = request_macro_arc_validation(
                        current_arc,
                ...
                if repaired is None:
                    last_error = (
                        "Macro arc repair exhausted its retry budget: "
                        f"{repair_error}"
                    )
                    break
            ...
            last_error = "The previous arc could not be repaired into a valid arc."
        return None, False
    ...
    def generate_batches(
        ...
        seed_beats=None,
        end_boundary_beat=None,
        include_later_phases=False,
    ):
        """Generate the plan or a phase suffix with an immutable prefix."""
        generated = list(seed_beats or []) if phase_number is not None else []
        ...
        if phase_number is not None:
            target_batches = [
                ...
                if (
                    phase_batch["phase"].get("phase_number") == phase_number
                    if not include_later_phases
                    else phase_batch["phase"].get("phase_number", 0) >= phase_number
                )
            ]
            if len(target_batches) != 1:
                if not include_later_phases or not target_batches:
                    raise ValueError(
                        f"Cannot regenerate unknown or split macro phase {phase_number}."
                    )
                matching_batches = [
                    phase_batch
                    for phase_batch in target_batches
                    if phase_batch["phase"].get("phase_number") == phase_number
                ]
            ...
            expected_prefix_length = target_phase["beat_start"] - 1
            if len(generated) != expected_prefix_length:
                raise ValueError(
                    f"Phase {phase_number} regeneration requires exactly "
                    f"{expected_prefix_length} immutable prefix beats."
                )
            phase_batches = target_batches
        for phase_batch in phase_batches:
            ...
            while attempt < BEAT_PHASE_GENERATION_ATTEMPTS:
                ...
                messages = build_beat_generation_messages(
                    ...
                    previous_phase_final_beat=(
                        generated[-1]
                        if current_phase["phase_number"] > 1 and generated
                        else None
                    ),
                    end_boundary_beat=end_boundary_beat,
                    macro_arc=macro_arc,
                ...
                try:
                    ...
                    if attempt >= BEAT_PHASE_GENERATION_ATTEMPTS:
                        ...
                        continue
                # Semantic beat validation is performed once per beat by the
                # forward validator. Generation only enforces structural rules.
                break
        ...
        if phase_number is not None and not include_later_phases:
            target_phase = phase_batches[0]["phase"]
    ...
    def generate_candidate(
        *,
        ...
        messages = build_beat_generation_messages(
            ...
            beat_instructions=beat_instructions,
            subject_information=subject_information,
            batch_start=beat_number,
            batch_end=beat_number,
            previous_beats=previous_finalized_beats,
            previous_phase_final_beat=(
                previous_finalized_beats[-1]
                if previous_finalized_beats
                else None
            ),
            end_boundary_beat=next_beat_job,
            macro_arc=macro_arc,
            current_phase=current_phase,
            phrase_exclusions=phrase_exclusions,
        )
    ...
    # checkpoint, and validation retries. A fresh arc is allowed only after
    # beat, phase, and same-arc full-process retry scopes are exhausted; a
    # rejected candidate itself is regenerated in place and never rewritten
    # locally by the validator. Any exhausted retry scope returns to the
    # nearest earlier creation scope instead of escaping as a fatal error.
    ...
    saved_arc_checked = False
    process_attempt = 0
    phase_retry_counts = {}
    force_clean_validation_state = bool(reset_validation_state)
    while True:
        try:
            ...
            macro_arc = active_macro_arc
            phase_number = error.phase_number
            if phase_number is None and macro_arc is not None:
            ...
            # Preserve only finalized phases before the failed phase. The
            # failed phase and every later phase must be regenerated from the
            # same macro arc because later generation depends on the new phase
            # output and state.
            if macro_arc is not None and phase_number is not None:
                phase_retry_count = phase_retry_counts.get(phase_number, 0)
                if phase_retry_count < BEAT_RETRY_ATTEMPTS:
                    phase_retry_counts[phase_number] = phase_retry_count + 1
                    phase = next(
                        (
                            item for item in macro_arc.get("phases", [])
                            if item.get("phase_number") == phase_number
                        ),
                        None,
                    )
                    if phase is not None:
                        prefix_end = int(phase["beat_start"]) - 1
                        try:
                            checkpoint = load_beat_validation_state(
                                validation_state_path
                            )
                            if not isinstance(checkpoint, dict):
                                raise ValueError(
                                    "No usable validation checkpoint was available "
                                    "for phase recovery."
                                )
                            prefix_beats = [
                                record["beat_text"]
                                for record in checkpoint.get(
                                    "finalized_beats", []
                                )[:prefix_end]
                            ]
                            regenerated_framework = generate_batches(
                                macro_arc,
                                phase_number=phase_number,
                                seed_beats=prefix_beats,
                                include_later_phases=True,
                            )
                            checkpoint = (
                                _truncate_beat_validation_checkpoint_to_phase_prefix(
                                    checkpoint,
                                    prefix_end,
                                    regenerated_framework,
                                    macro_arc,
                                )
                            )
                            save_beat_validation_state(
                                checkpoint,
                                validation_state_path,
                            )
                            force_clean_validation_state = False
                            preserved_prefix = (
                                f"beats 1-{prefix_end}"
                                if prefix_end
            ...
            if macro_arc is not None and process_attempt < BEAT_PROCESS_ATTEMPTS:
                process_attempt += 1
            ...
            for stale_path in (
                story_arc_path,
                get_story_arc_hash_path(story_arc_path),
                validation_state_path,
            ):
                try:
                    if os.path.exists(stale_path):
...
def _validate_combined_continuity_schema(candidate):
    ...
    def validate_subject(path, record):
        ...
        for key in list_subject_fields:
            ...
            if not isinstance(values, list):
                ...
                errors.append(f"{path}.{key} (must contain strings)")
        wardrobe = record.get("wardrobe")
        if wardrobe is not None:
            if not isinstance(wardrobe, dict):
                errors.append(f"{path}.wardrobe (must be an object)")
            else:
                for key in wardrobe:
                    if key not in _WARDROBE_FIELDS:
                        errors.append(f"{path}.wardrobe.{key}")
                for key in _WARDROBE_FIELDS:
                    if key in wardrobe and not isinstance(wardrobe[key], str):
                        errors.append(
                            f"{path}.wardrobe.{key} (must be a string)"
                        )
    ...
    if subjects is not None:
        if isinstance(subjects, dict):
            for name, record in subjects.items():
                validate_subject(f"subjects.{name}", record)
        elif isinstance(subjects, list):
            for index, record in enumerate(subjects):
    ...
    if errors:
        ...
        raise ValueError("SCHEMA ERROR: " + ", ".join(unique_errors))
...
def request_combined_continuity(
    ...
    for attempt in range(1, attempts + 1):
        ...
        if attempt > 1:
            retry_feedback = schema_feedback
            if retry_feedback:
                messages[-1]["content"] += "\n\n" + retry_feedback
            else:
                messages[-1]["content"] += (
        ...
        try:
            ...
            schema_feedback = (
                str(error).strip()
                if str(error).strip().startswith("SCHEMA ERROR:")
                else ""
            )
            if raw_received:
    ...
    opening_state = request_continuity_opening_state(
        ...
        history_metadata=history_metadata,
        subject_definitions=subject_definitions,
        ending_scene=ending_scene or h3_prompt,
    )
    return {
        "reduced_state": reduced_state,
...
def _open_h3_continuation_description(
    ...
    if summary_text:
        ...
        if prefix_match is not None:
            ...
            if conditioning_mode == "clean_refresh":
                opener = (
                    "[Shot 1] The opening composition is already established by the supplied first frame. "
